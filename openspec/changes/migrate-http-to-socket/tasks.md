...
- [ ] T019 在 socket 动作导致手牌结束时，确保手牌结果归档、钱包更新、房间状态更新成功后再广播。
- [ ] T020 广播 `hand.settled` 给房间订阅者。
- [ ] T021 向相关用户发送 `wallet.updated`，提示前端刷新钱包。
- [ ] T022 编写结算后 `hand.settled` 与 `wallet.updated` 事件测试。
- [ ] T023 编写持久化失败时不广播成功事件的测试。
