...
当前目标是 Vue 前端 + Go 后端的多人德州扑克对战 v1，保留独立规则引擎测试页，新增账号登录、虚拟金币、房主建房邀请码加入、正式多人轮流操作，以及每手牌结果保存与展示；运行、开发和测试统一使用 MySQL。本版本计划将正式多人对战从 HTTP 轮询/HTTP 动作提交升级为 socket 实时同步与 socket command。
